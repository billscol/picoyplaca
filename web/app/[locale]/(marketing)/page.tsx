    ...
    <>
      ...
      <section className="bg-hero-wash">
        <div className="mx-auto flex max-w-3xl flex-col items-center gap-6 px-4 py-20 text-center sm:py-28">
          <span className="animate-in fade-in zoom-in inline-flex items-center gap-1.5 rounded-full border border-border bg-white px-3 py-1 text-xs font-medium text-muted-foreground duration-500">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 fill-mode-both flex flex-wrap items-center justify-center gap-3 pt-2 duration-700 delay-300">
            <Button
              size="lg"
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-3">
          {models.map((model, i) => {
            ...
            return (
              <Card
                key={model}
                className="card-hover-lift animate-in fade-in slide-in-from-bottom-4 fill-mode-both border-border p-2 shadow-(--shadow-subtle) duration-500"
                style={{ animationDelay: `${i * 100}ms` }}
              >
                <CardHeader>
                  <span
                    className={`mb-2 flex size-10 items-center justify-center rounded-xl transition-transform duration-300 hover:scale-110 ${MODEL_CHIP_CLASS[model]}`}
                  >
                    <Icon className="size-5" />
                  </span>
                  <CardTitle>{tCities(`restriction_model.${model}`)}</CardTitle>
                  <CardDescription>{tHow(`${model}_desc`)}</CardDescription>
                </CardHeader>
              </Card>
            );
          })}
        ...
        <section className="bg-section-wash">
          <div className="mx-auto max-w-6xl px-4 py-16">
                    ...
                    <Card className="card-hover-lift h-full border-border p-2">
                      <CardHeader>
                        <div className="flex items-start justify-between">
                          <span
                            className={`flex size-10 items-center justify-center rounded-xl transition-transform duration-300 group-hover:scale-110 ${MODEL_CHIP_CLASS[city.restriction_model]}`}
                          >
            ...
            <div className="mt-8 flex justify-center">
              <Button variant="outline" className="rounded-full px-6" render={<Link href="/ciudades" />}>
                {tCitiesSection("cta")}
              </Button>
            </div>
          </div>
      ...
      <section className="mx-auto max-w-3xl px-4 py-16">
        ...
        <Card className="card-hover-lift mt-10 border-border p-2 shadow-(--shadow-subtle)">
          <Accordion>
            {faqItems.map((item, i) => (
      ...
      <section className="mx-auto max-w-4xl px-4 pb-24">
        <div className="bg-hero-wash card-hover-lift flex flex-col items-center gap-4 rounded-2xl border border-border px-6 py-14 text-center shadow-(--shadow-subtle)">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            ...
            <Button size="lg" variant="outline" className="rounded-full px-6" render={<Link href="/register" />}>
              {tCta("cta_secondary")}
            </Button>
          </div>
        </div>
